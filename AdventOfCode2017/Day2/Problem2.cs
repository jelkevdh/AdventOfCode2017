...
namespace AdventOfCode2017.Day2 {
  public class Problem2 : Problem {
    /// <summary>
    /// Initializes a new instance of the <see cref="Problem2"/> class.
    /// </summary>
    ...
    public void Solve(string input) {
      ...
      foreach (var row in rows) {
        ...
        foreach (var column in columns) {
          ...
          foreach (var colValue in values) {
            if (colValue > value) {
              if (Math.IEEERemainder(colValue, value) == 0) {
                result += (colValue / value);
                found = true;
                break;
              }
            } else {
              if (Math.IEEERemainder(value, colValue) == 0) {
                result += (value / colValue);
                found = true;
                break;
              }
            }
